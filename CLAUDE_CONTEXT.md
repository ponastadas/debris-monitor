...
> Last updated: 2026-05-02 (session 28 — tracker search/load bug fix: SatelliteSearchController all local-DB queries now require whereHas('currentTle') so only loadable satellites appear in dropdown; SatelliteController show/orbit error response changed from bare JSON to standard {success,data} envelope via $this->error(); frontend loadAndTrack catch now reads err.response?.data?.error?.message before falling back to generic message; satellites:sync chunk size fixed from 8000→7000 (satellite rows have 9 columns not 8, was exceeding MySQL 65535 placeholder limit); ResizeObserver stub added to vitest setup.js; 45 backend tests + 42 frontend tests all pass; catalog synced to 30801 satellites).
