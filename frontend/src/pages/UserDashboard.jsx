...
const PLANS = [
  { key: 'starter',    label: 'Starter',    price: '$29/mo',  calls: '10,000/day',  webhooks: false },
  { key: 'pro',        label: 'Pro',        price: '$99/mo',  calls: '100,000/day', webhooks: true  },
  { key: 'enterprise', label: 'Enterprise', price: '$499/mo', calls: 'Unlimited',   webhooks: true  },
];
...
function BillingTab({ user }) {
  ...
  const subscribe = async (plan) => {
    ...
    try {
      ...
      const fresh = await client.get('/billing/plan');
      setBilling(fresh.data.data);
      await refreshUser();
    } catch (err) {
  ...
  const cancel = async () => {
    if (!confirm('Cancel your subscription? You will be downgraded to the free tier.')) return;
    try {
  ...
  return (
    <div>
      {/* Current plan */}
      <div style={{ ...S.card, marginBottom: 24 }}>
        <div style={{ fontSize: 11, color: '#8b949e', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>
          Current Plan
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, justifyContent: 'space-between' }}>
          ...
          {currentPlan !== 'free' && (
            <button style={{ ...S.btn(false), color: '#f85149', borderColor: 'rgba(248,81,73,0.3)' }} onClick={cancel}>
              CANCEL
            </button>
          )}
        </div>
        ...
        {PLANS.map((plan) => {
          ...
          return (
            <div key={plan.key} style={{
              ...
              <div style={{ fontSize: 18, color: '#e6edf3', marginBottom: 12 }}>{plan.price}</div>
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 16px', fontSize: 12, color: '#8b949e', lineHeight: 1.8 }}>
                <li>✓ {plan.calls}</li>
                <li>{plan.webhooks ? '✓ Webhooks' : '— No webhooks'}</li>
      ...
      <p style={{ fontSize: 11, color: '#484f58', marginTop: 16 }}>
